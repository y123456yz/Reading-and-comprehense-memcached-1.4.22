...
struct settings {
    ...
    int evict_to_free; //标记memcached是否允许LRU淘汰机制。默认是可以的。可以通过-M选项禁止
...
/* temp */
//该item还在slab的空闲队列里面，没有分配出去 见do_slabs_free
#define ITEM_SLABBED 4
//该item 插入到LRU队列后，被worker线程访问过
/* Item was fetched at least once in its lifetime */
#define ITEM_FETCHED 8 //当客户端get该item的时候会置ITEM_FETCHED
/* Appended on fetch, removed on LRU shuffling */
#define ITEM_ACTIVE 16 //当客户端get该item的时候会置ITEM_FETCHED
...
typedef struct _stritem {
    ...
    unsigned short  refcount;  //在do_item_link中增加
...
struct slab_rebalance { //存储在slab_rebal   赋值可以参考slab_rebalance_start  重分页的时候用
    ...
    uint32_t busy_items;//是否worker线程在引用某个item,记录引用数   slab迁移过程中正在被其他线程访问的trunk数，赋值见slab_rebalance_move
    uint32_t rescues;
